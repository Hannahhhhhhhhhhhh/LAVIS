...
class MoEPrompt(nn.Module):
    ...
    def _weighted_select_expert(self, expert_ids, prob_gate_i):
        ...
        select_qts = [self.query_token_candidates[expert_id] for expert_id in expert_ids]
    ...
    def _forward_gate_single_token(self, x):
        ...
        out = [self._weighted_select_expert(gate[i], prob_gate[i])  for i in range(gate.shape[0])]
